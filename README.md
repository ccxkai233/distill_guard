...
核心定位是 **过滤 + 小量放行**,而不是"发现即封号":对疑似批量采集的流量只放一小部分进上游,其余返回一个仿真上游形态的假错误;对正常聊天 / RAG 流量零干扰、零开销直通。
...
| 很多报错是网关主动返回的 | **主动假错**:被拒请求根本不到上游,返回一个仿真上游形态的错误响应 |
...
> **健康探针**:`GET /healthz` 由网关本地应答(不转发上游),返回 `{"status":"ok","shadow":<bool>}`,供负载均衡 / compose healthcheck 使用。
...
> **上线前务必核对假错的状态码和 body**,替换成你实际上游返回的原文,让它尽量贴近真实上游的过载错误——否则对手一测就能区分出"这是被网关拦的"还是"上游真的过载"。注意假错经网关 `JSONResponse` 重新序列化,响应头顺序、`request-id` 等上游专有头、JSON 空白格式都无法做到与上游**逐字节**一致;要更逼真需在反代层透传/伪造这些头。
